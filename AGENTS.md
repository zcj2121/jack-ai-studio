...
1. **Provider**
   - 翻译（Translation）：模型服务提供方。
   - 当前语境解释（Context Explanation）：在当天代码中，Provider 是后端负责接收模型请求并返回结果的服务边界。
...
- 有 Git Commit 建议
- 有 5～10 个英文术语
- `今日英文术语` 中每个术语必须分成两层说明：
  - `翻译（Translation）`：给出这个词或短语的直接中文翻译。
  - `当前语境解释（Context Explanation）`：说明它在当天学习内容和 Jack AI Studio 代码中的具体含义；不能只重复直译。
- 不要求用户额外寻找课程

### 9.1 Sprint 总结规范

每完成一个 Sprint，必须新增一份总结文档：

```text
docs/learning/sprint-XX-summary.md
```

Sprint 总结不是 Daily 文档的简单拼接，而是要把一个阶段学到的内容串成一条完整链路。至少包含：

- Sprint 目标、范围、完成的 Day 和阶段产出。
- 核心概念总表：每个概念说明“是什么、为什么需要、在 Jack AI Studio 中哪里使用”，必要时补充前端类比和容易混淆的边界。
- 主流程图：使用 Mermaid 串起用户操作、Web State、统一请求层、BFF、FastAPI、Provider、数据或响应展示；涉及多个流程时拆成主流程和异常流程。
- 代码与架构对应关系：说明关键目录、模块职责、公共契约和数据流向，标出哪些能力属于 Web、API、Provider 或未来的数据层。
- 产品能力回顾：用用户实际能完成的任务描述阶段成果，不只罗列技术名词。
- 验证证据：记录质量门禁、自动化测试、浏览器/运行时验收、移动端检查和未调用真实服务的边界。
- Sprint 复习题：至少 5 道编号问题，覆盖概念区别、流程顺序、代码职责和失败处理；用户回答后才能把 Sprint 标记为已完成。
- 遗留问题、风险和下一 Sprint 的承接点，明确哪些内容本 Sprint 没有提前实现。

总结文档应优先使用中文，保留必要的英文术语；流程图节点按初学者可读的顺序表达。更新 `PROGRESS.md` 时，只有 Sprint 总结和复习问答均完成，才可以把当前 Sprint 切换到下一 Sprint。
